...
def main():
    ...
    if len(ps)<20:e.append('need >=20 profiles')
    for p in ps:
    ...
    if len(ids)<18 or len(ids)!=len(set(ids)):e.append('behavioral corpus invalid')
    ...
    if len(cal)<12 or {x['reviewer'] for x in cal}!=REV:e.append('reviewer calibration corpus invalid')
    for x in REQ:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if meta.get('version')!='1.2.0' or not meta.get('migration_history'):e.append('migration history/version not recorded')
