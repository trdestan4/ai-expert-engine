...
def main():
    e=[];s=skills()
    if len(s)!=50:e.append('skill count changed')
    ...
    if len(ps)<24 or len({x.get('dimension') for x in ps})<5:e.append('need >=24 profiles across five composable dimensions')
    for p in ps:
    ...
    if len(ids)<40 or len(ids)!=len(set(ids)):e.append('behavioral corpus invalid')
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if not (t/'scripts/creative_experience_checks.py').exists():e.append('installer missing creative experience checker')
        ...
        if meta.get('version')!='1.4.0' or len(meta.get('migration_history',[]))<3:e.append('migration history/version not recorded through v1.4')
        if not (t/'.ai-expert-engine/evidence/reviews.jsonl').exists():e.append('legacy review store not migrated')
        if not (t/'.ai-expert-engine/state/creative-engineering-v1.4.json').exists():e.append('v1.4 creative migration marker missing')
        ...
        for rel in ('scripts/release_gate.py','scripts/runtime_contract.py','scripts/design_quality_checks.py','scripts/creative_experience_checks.py','scripts/routing_report.py','.github/workflows/ai-expert-release-gate.yml'):
    ...
    print(f'hardening validation passed: 50 skills, 6 reviewers, {len(ids)} behavioral, {len(drift)} drift, {len(cal)} calibration, {len(bench)} repo benchmarks, {len(ps)} profiles');return 0
