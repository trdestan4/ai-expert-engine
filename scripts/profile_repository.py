...
def collect(root:Path,max_files:int=50000)->dict[str,Any]:
    root=root.resolve();deps={};dep_sources={};files=[];text_signals=[];seen=0;truncated=False
    for p in root.rglob('*'):
        ...
        if p.name=='package.json' and len(rel.parts)<=8:
            pkg=read_json(p)
    ...
    return {'root':str(root),'dependencies':sorted(deps),'dependency_versions':deps,'dependency_sources':dep_sources,'files':sorted(files),'files_profiled':len(files),'files_seen_before_stop':seen,'truncated':truncated,'truncation_reason':f'max_files={max_files}' if truncated else None,'signals':sig,'text_signals':'\n'.join(text_signals)[:1000000]}
def main():
    ap=argparse.ArgumentParser();ap.add_argument('root',nargs='?',default='.');ap.add_argument('--output');ap.add_argument('--max-files',type=int,default=50000);ns=ap.parse_args();txt=json.dumps(collect(Path(ns.root),ns.max_files),indent=2,ensure_ascii=False)
