...
def evidence_hash(paths):
    ...
    return h.hexdigest()
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--candidate',required=True);ap.add_argument('--environment',required=True);ap.add_argument('--risk',required=True,choices=['R0','R1','R2','R3','R4']);ap.add_argument('--decision',required=True,choices=['GO','GO WITH CONDITIONS','HOLD','NO-GO']);ap.add_argument('--evidence',action='append',required=True);ap.add_argument('--reviews',default='.ai-expert-engine/evidence/reviews.jsonl');ap.add_argument('--condition',action='append');ap.add_argument('--output',default='.ai-expert-engine/evidence/release-decision.json');n=ap.parse_args();reviews=Path(n.reviews);openb=0
    if reviews.exists():openb=sum(1 for x in reviews.read_text().splitlines() if x.strip() and (lambda r:r.get('candidate')==n.candidate and r.get('blocker') and r.get('status')=='open')(json.loads(x)))
    d={'candidate':n.candidate,'environment':n.environment,'risk':n.risk,'decision':n.decision,'evidence_hash':evidence_hash(n.evidence),'review_store':str(reviews) if reviews.exists() else None,'open_blockers':openb,'conditions':n.condition or[],'generated_at':datetime.now(timezone.utc).isoformat(),'expires_at':None};p=Path(n.output);p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(d,indent=2)+'\n')
