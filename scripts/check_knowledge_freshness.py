...
REQUIRED_OWNERS={'master-agent','react-nextjs','frontend-engineering','database-data','identity-access','security','accessibility','performance','api-engineering','ai-engineering','observability-sre','seo','integrations','ecommerce','saas-platform','devops-deployment','git-delivery','design-reconstruction','creative-frontend','animation-engineering','threejs-webgl','realtime-shaders','three-d-asset-pipeline','visual-qa'}
def main():
    ...
    for x in d['sources']:
        ...
        if n.online:
            try:
                req=urllib.request.Request(x['url'],headers={'User-Agent':'ai-expert-engine-freshness/1.4'});body=urllib.request.urlopen(req,timeout=20).read(1000000).decode('utf-8','ignore').lower()
