...
**Slow PostgreSQL query:** `database-data`; future `performance` only for broader performance review.

**Large live schema migration:** `database-data` + `task-planning`; future `release-readiness` for production cutover.
...
**Private image upload:** `storage-media`; add `identity-access` for ownership policy, `database-data` for metadata/RLS, `realtime-async` for heavy processing.

**Payment checkout:** `integrations` owns provider/payment lifecycle; `api-engineering` owns public application API contract; `database-data` owns order/payment persistence constraints; future `security` reviews threat surface.
...
Do not duplicate general exploit review here; future `security` owns threat-model/audit. Do not duplicate deployment/monitoring mechanics; future `devops-deployment` and `observability-sre` own those.
